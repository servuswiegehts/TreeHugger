...
, connected   (false)
{
	try {
		...
		std::cerr << "QTUIO: could not bind to UDP port " << port << std::endl << std::endl;
		socket = NULL;
	}
	...
	if (socket!=NULL) {
		if (!socket->IsBound()) {
			delete socket;
			socket = NULL;
		} else std::cout << "QTUIO: listening to TUIO messages on UDP port " << port << std::endl << std::endl;
	}	
}
...
void TuioClient::ProcessBundle( const ReceivedBundle& b, const IpEndpointName& remoteEndpoint) {
	...
	try {
		...
		std::cerr << "QTUIO: malformed OSC bundle: " << e.what() << std::endl;
	}
...
void TuioClient::ProcessMessage( const ReceivedMessage& msg, const IpEndpointName& remoteEndpoint) {
	try {
		...
		std::cerr << "QTUIO: error parsing TUIO message: "<< msg.AddressPattern() <<  " - " << e.what() << std::endl;
	}
}
