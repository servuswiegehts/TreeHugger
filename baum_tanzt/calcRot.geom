...
{	
	...
	float m = 1.0; // mass of branch
	
	float my = 6; // determined by thickness of branch
	...
	//Windkraft
	//
	float sigma = 0.0980; // viscosity coefficient of air
	vec3 v = vec3(1.0, 0.0, 0.0); // Velocity of wind
	...
	//Restoration force
	//
	float k = 8.5; // rigidity of branch (determined by thickness)
	vec3 passAngleNorm = vec3(1.0);
	
	if (frameCount == 0)
	{
		passAngleNorm = (posEnd - posStart).xyz;
	}
	else
	{
